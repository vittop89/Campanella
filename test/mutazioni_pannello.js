...
const MUTAZIONI = [
  ...
  { nome: 'la chiusura non controlla che il modulo sia chiuso davvero',
    ...
    a: '' },
  { nome: 'chiusura e controllo falliti tutti e due: il modulo conta come chiuso',
    da: '      try { ancoraAperto = form.isAcceptingResponses(); } catch (e6) { ancoraAperto = chiusuraFallita; }',
    a: '      try { ancoraAperto = form.isAcceptingResponses(); } catch (e6) { ancoraAperto = false; }' },
  { nome: 'la chiusura scattata non lascia il segno',
    da: "    memoria.chiusi[r.id + '|' + _panAnnoDellaData_(scadenza)] = scadenza;",
    a: '' },
  { nome: 'il segno della chiusura va all\'anno dello script, non a quello della data',
    da: "    memoria.chiusi[r.id + '|' + _panAnnoDellaData_(scadenza)] = scadenza;",
    a: "    memoria.chiusi[r.id + '|' + _panAnno_()] = scadenza;" },
  { nome: 'un modulo cancellato lascia la sua scadenza in sospeso',
    da: "      _panScriviStato_(foglio, r, 'non si apre piu\\' (cancellato?): chiusura saltata', null);\n      delete memoria.scadenze[r.id];",
    a: "      _panScriviStato_(foglio, r, 'non si apre piu\\' (cancellato?): chiusura saltata', null);" },
  { nome: 'il trigger scattato poco prima di mezzanotte salta la riga',
    da: "  var oggi = Utilities.formatDate(new Date(new Date().getTime() + 30 * 60 * 1000), _panFuso_(), 'yyyy-MM-dd');",
    a: '  var oggi = _panOggi_();' },
  { nome: 'un tentativo durante l\'ultimo giorno chiude la riga prima del tempo',
    da: '    if (!scadenza || oggi <= scadenza) continue;',
    a: '    if (!scadenza || oggi < scadenza) continue;' },
  { nome: 'una chiusura non riuscita non si riprova',
    da: '  if (riprovare) _panRiprogramma_(e);',
    a: '  if (false) _panRiprogramma_(e);' },
  { nome: 'il trigger scattato resta in elenco',
    da: 'function _panRiprogramma_(e) {\n  _panTogliScattato_(e);',
    a: 'function _panRiprogramma_(e) {' },
  ...
  { nome: 'un modulo scollegato a mano viene ricollegato',
    da: '  } else if (foglioDiQuestAnno && !memoria.annullati[chiave] && _panGiaCollegatoUnaVolta_(idFoglio)) {',
    a: '  } else if (false) {' },
  ...
  { nome: 'le chiusure delle righe non riuscite si perdono',
    da: '    if (s >= oggi) fuori.date[s] = _panScadenza_(s);',
    a: '    if (false) fuori.date[s] = _panScadenza_(s);' },
  ...
  { nome: 'nessun limite di tempo',
    ...
    a: '    if (false) {' },
  { nome: 'il limite di tempo parte dopo la conferma, non dal clic sul menu',
    da: '  var inizio = inizioEsecuzione || new Date().getTime();',
    a: '  var inizio = new Date().getTime();' },
  { nome: 'l\'anteprima a tempo finito promette di ripartire dalle righe non guardate',
    da: '    if (daFare.length && davvero) {',
    a: '    if (daFare.length) {' },
  ...
  { nome: '"pronti" degli anni passati si accumulano',
    da: '      m.pronti = _panSoloAnno_(m.pronti, anno);',
    a: '' },
  ...
  { nome: 'la chiusura diventata data si rilegge nel fuso sbagliato',
    da: "    return Utilities.formatDate(valore, _panFuso_(), 'dd/MM');",
  ...
  { nome: 'la scheda Istruzioni si accoda invece di riscriversi',
    da: '  var s = _panSchedaIstruzioni_(ss);\n  s.clear();',
    a: '  var s = _panSchedaIstruzioni_(ss);' },
  ...
  { nome: 'il foglio gia\' collegato con lo stesso nome non viene adottato',
    da: '    if (giaCollegato && _panNomeDiUnFile_(giaCollegato) === nomeFoglio) {',
    a: '    if (false) {' },
  { nome: 'i permessi a meta\' non fermano niente',
    da: '  if (davvero && !_panPermessiCompleti_()) {',
    a: '  if (false) {' },
